...
def add_new_employee(request):
    try:
        ...
        department_id = int(request.json['department_id'])
    except KeyError:
    ...
    cur.execute(
        """insert into employee.employee(first_name,last_name,manager_id,joining_date,role_id,department_id,daily_salary) values(
        '{first_name}','{last_name}',{manager_id},'{joining_date}',{role},{department},{salary});""".format(
            first_name=first_name, last_name=last_name, manager_id=manager_id, joining_date=joining_date, role=role,
            department=department_id,salary=salary))
    cur.execute('commit;')
    cur.execute('select max(employee_id) from employee.employee ;')
    emp_id = cur.fetchone()[0]
    return {"status": "Employee added", "id": emp_id, "Error": ''}


def credit_employee_salary(request):
    cnx = connect_db()
    cur = cnx.cursor()
    try:
        acc_id = request.json["accountant_id"]
    ...
    if role.lower() != 'accountant':
        status = {"Error": "Account ID submitted is not an accountant. Only Accountants can credit salary"}
        return status
    ...
    if result:
        ...
        if 0 < salary_amount <= daily_salary:
            ...
            cur.execute('commit;')
            status = {"status": "Salary Credited","id":emp_id}
        else:
            status = {"Error": "Salary amount cannot be 0 or greater than the daily salary"}
...
def get_employee_salary_history_by_date(emp_id, date):
    cnx = connect_db()
    cur = cnx.cursor()
    cur.execute("""select concat(e.first_name," ",e.last_name) as employee_name from employee.employee e where 
                        e.employee_id = {emp_id}""".format(emp_id=emp_id))
